 ...
 *
 * @author db117
 * @since 2020-12-18 18:34:52
 **/

public class Solution1403 {
    public static void main(String[] args) {
        Solution solution = new Solution1403().new Solution();
    ...
    class Solution {
        public List<Integer> minSubsequence(int[] nums) {
            ...
            // 在校验一下
            // 当出现这种情况是,把最后一个数字加在了leftSum上
            if (leftSum >= rightSum &&
                    leftSum - nums[right] >= rightSum) {
                right--;
            }
            // 这种情况是最后一个数字加在了right上
            if (rightSum > leftSum &&
                    rightSum - nums[right] > leftSum) {
                right++;
            }
