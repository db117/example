 ...
 *
 * @author db117
 * @since 2021-02-18 16:07:55
 **/

public class Solution_1024 {
    public static void main(String[] args) {
        Solution solution = new Solution_1024().new Solution();
    ...
    class Solution {
        public int videoStitching(int[][] clips, int T) {
            ...
            Arrays.sort(clips, Comparator.comparing(ints -> ints[0] * 100 + ints[1]));
            int res = 0;
        ...
        // 找到小于等于开始时间的最大结束时间
        int maxEnd(int[][] clips, int start) {
            int max = start;
            for (int[] clip : clips) {
                if (clip[0] > start) {
                    break;
