 ...
 *
 * @author db117
 * @since 2021-01-04 16:30:10
 **/

public class Solution1710 {
    public static void main(String[] args) {
        Solution solution = new Solution1710().new Solution();
        System.out.println(solution.maximumUnits(new int[][]{
                // [5,10],[2,5],[4,7],[3,9]
                {5, 10},
                {2, 5},
                {4, 7},
    ...
    class Solution {
        public int maximumUnits(int[][] boxTypes, int truckSize) {
            // 根据装载的单元数量排序
            Arrays.sort(boxTypes, 0, boxTypes.length, Comparator.comparingInt(value -> value[1]));
            ...
            while (truckSize > 0 && index >= 0) {
                ...
                // 每一种最多能装的数量
                res += value * Math.min(num, truckSize);
                truckSize -= num;
                index--;
            }
